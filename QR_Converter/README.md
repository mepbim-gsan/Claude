...
- **QRコード画像 → 中身を確認**
  - QRコード画像をペースト（Ctrl+V）／ドラッグ&ドロップ／ファイル選択すると、読み取った内容をURL・テキストと自動判定して表示
...
1. 上部の入力欄にURLやテキストを入力・ペースト、またはQRコード画像をペースト／ドロップ
...
- [qrcode](https://github.com/soldair/node-qrcode)（QRコード生成、CDN経由）
